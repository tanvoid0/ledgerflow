...
policy() {
  ...
  docker exec ledgerflow-redpanda rpk topic create "$topic" -p 3 -r "$RF" "${isr[@]}" -c "retention.ms=$retention_ms" > /dev/null 2>&1 || true   # exists already: fine
...
docker exec ledgerflow-redpanda rpk topic create ledgerflow.balance.snapshots.v1 -p 3 -r "$RF" "${isr[@]}" \
