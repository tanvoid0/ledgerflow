...
- A remote-chunking run costs one serialised round trip per 100 items instead of one dispatch per partition: 4,000 messages against 8 for the same 200,000 rows, at {{R: chunking}} bytes each — most of that bill is the `StepContribution` graph riding along, not the item itself.
...
```text
in-JVM (cluster):     {{R: in-jvm}}
remote, 4 workers:    {{R: remote}}
```
