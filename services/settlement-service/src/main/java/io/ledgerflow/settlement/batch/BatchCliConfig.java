...
class BatchCliConfig {
    ...
    // Running is not stranded: a live run's step rows move on every chunk commit, so only one quiet for longer
    // than the remote manager's own poll timeout (lineItemsStep, 10 min) counts as dead. Without that, the
    // */10 tick recovered a 3-second-old run under its own manager.
    ...
    private static int recoverStranded(CommandLineJobOperator operator, JobRepository jobRepository) {
        LocalDateTime cutoff = LocalDateTime.now().minus(QUIET);
        for (JobExecution execution : jobRepository.findRunningJobExecutions("nightlySettlementJob")) {
            if (lastProgress(execution).isBefore(cutoff)) {
    ...
    private static LocalDateTime lastProgress(JobExecution execution) {
        return Stream.concat(Stream.of(execution.getStartTime(), execution.getLastUpdated()),
                        execution.getStepExecutions().stream().map(StepExecution::getLastUpdated))
